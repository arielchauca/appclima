import { GET_CITY_BY_NAME, DELETE_CITY, GET_CITY_BY_ID, GET_DAYS_BY_CITY } from "./types";
...
export const getCityById = (id) => async (dispatch) => {
  try{
    let city = await axios.get(`https://api.openweathermap.org/data/2.5/weather?id=${id}&appid=${API_KEY}&units=metric`);
...
export const getDaysCity = (lat, long) => async (dispatch) => {
  try{
    const city = await axios.get(`https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${long}&exclude=current,minutely,hourly,alerts&appid=${API_KEY}&units=metric`)
    dispatch({
